...
def place_dihedral(
    ...
    if not use_torch:
        ...
        # Ensure input tensors require gradients if they are part of the computation graph
        if a.requires_grad or b.requires_grad or c.requires_grad:
            torsion_angle.requires_grad_()
            bond_angle.requires_grad_()
            bond_length.requires_grad_()

        unit_vec = lambda x: x / (torch.linalg.norm(x, dim=-1, keepdim=True) + 1e-8)
...
def nerf_build_batch(
    ...
    # (batch, seq, 3)
    # CRITICAL FIX: Initialize with requires_grad=True to allow gradient flow
    init_coords_np = np.array([N_INIT, CA_INIT, C_INIT])
    coords = (
        torch.tensor(init_coords_np, dtype=phi.dtype, device=phi.device)
        .repeat(batch, 1, 1)
    )
    # Manually set requires_grad after creation and repetition
    coords.requires_grad_()

    assert coords.shape == (batch, 3, 3), f"Mismatched shape: {coords.shape}"

    # perform broadcasting of bond lengths
    # CRITICAL FIX: ensure_tensor should not set requires_grad=False
    ensure_tensor = (
        lambda x: torch.tensor(x, dtype=phi.dtype, device=phi.device).expand(phi.shape)
    ...
    # --- 개선: torch.cat 반복 호출 대신 리스트에 좌표를 모읍니다 ---
    coords_list = [coords]

    for i in range(phi.shape[1]):
        # 이전 3개 좌표를 가져옵니다.
        # 루프의 첫 반복에서는 초기 coords 텐서에서, 이후에는 coords_list의 마지막 3개에서 가져옵니다.
        p_prev3, p_prev2, p_prev1 = (coords_list[-3], coords_list[-2], coords_list[-1]) if i > 0 else (coords[:, -3, :], coords[:, -2, :], coords[:, -1, :])
        ...
        # 생성된 좌표를 (batch, 3) 형태로 리스트에 추가합니다.
        coords_list.extend([n_coord, ca_coord, c_coord])

    # --- 개선: 루프 종료 후 한 번만 torch.stack과 view를 사용하여 최종 텐서를 생성합니다 ---
    # coords_list의 첫 항목은 (batch, 3, 3)이므로 펼쳐서 (batch, 3) 3개로 만듭니다.
    final_coords_list = []
    final_coords_list.extend([coords_list[0][:, i, :] for i in range(3)])
    final_coords_list.extend(coords_list[1:])

    # (batch, num_atoms, 3) 형태로 만듭니다.
    coords_final = torch.stack(final_coords_list, dim=1)
    
    assert coords_final.shape[-1] == 3 and coords_final.shape[0] == batch
